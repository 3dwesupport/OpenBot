...
def get_dataset_list(dir_path , id):
    return [get_dataset_info(dir_path, name, id) for name in listdir(dataset_dir, id,  dir_path)]
...
def get_dir_info(dir_path , id):
    files = []
    list1 = listdir(dataset_dir,id, dir_path)
    for basename in list1:
        info = get_info(id+"/"+dir_path, basename)
        if info:
            files.append(info)
    return files
