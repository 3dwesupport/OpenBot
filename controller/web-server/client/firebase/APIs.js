    ...
    try {
        ...
        if (docDetails === null) {
            return await addDoc(collection(db, tables.subscription),
                ...
                return {planType: Constants.free, planEndDate: endDate, planStartDate: startDate}
            })
        } else {
            const dateObject = new Date(docDetails?.data.sub_end_date.seconds * 1000 + docDetails?.data.sub_start_date.nanoseconds / 1e6)
